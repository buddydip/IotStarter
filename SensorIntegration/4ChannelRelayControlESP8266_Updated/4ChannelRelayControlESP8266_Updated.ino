...
const char *mqtt_topic = "<mqtt_tpoic>";
const char *nodeID = "<node_name>";
...
void MQTTcallback(char* topic, byte* payload, unsigned int length)
{
  ...
  // Fetch values
  String switchName = doc["SwitchID"];
  int switchState = doc["SwitchState"];

  //Check switch to toggle
  if (switchName.equals(switch1name))
  {
    controlSwitch(Switch1, switchState);
  }
  if (switchName.equals(switch2name))
  {
    controlSwitch(Switch2, switchState);
  }
  if (switchName.equals(switch3name))
  {
    controlSwitch(Switch3, switchState);
  }
  if (switchName.equals(switch4name))
  {
